...
export default function PerfilProfesional({ userData }) {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'enfermeros', user.uid)).then(snap => {
      if (snap.exists()) setEstadoPerfil(snap.data().estado || null)
    })
  }, [])
    ...
    if (estadoPerfil === 'activo') {
      return (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#dcfce7', color: '#166534', padding: '6px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>
          <i className="fa-solid fa-certificate"></i> Matrícula Verificada
        </div>
      )
    }
    if (estadoPerfil === 'pendiente_validacion' || estadoPerfil === 'pendiente_verificacion') {
      return (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#fef3c7', color: '#92400e', padding: '6px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>
          <i className="fa-solid fa-clock"></i> Matrícula en revisión
        </div>
      )
    }
    if (estadoPerfil === 'suspendido') {
      return (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#fee2e2', color: '#b91c1c', padding: '6px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>
          <i className="fa-solid fa-ban"></i> Cuenta suspendida
        </div>
      )
    }
    return null
  ...
  const eliminarCuentaDefinitivo = async (user) => {
    const db = getFirestore();
    await deleteDoc(doc(db, 'enfermeros', user.uid));
    ...
    <>
          ...
          <div style={{ background: 'white', borderRadius: '24px', padding: '30px 24px', maxWidth: '340px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            {pedirPassword && (
              <div style={{ marginBottom: '16px', textAlign: 'left' }}>
                ...
                />
                <button onClick={handleEliminarConPassword} disabled={eliminando || !password}
            ...
            <button onClick={() => { setModalVisible(false); setPedirPassword(false); setPassword(''); }} disabled={eliminando}
      ...
      <div style={{ padding: '20px 20px 120px 20px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div style={{ background: 'var(--blanco)', borderRadius: '24px', padding: '30px 20px', textAlign: 'center', boxShadow: 'var(--shadow-soft)', marginBottom: '25px' }}>
          ...
          {cargando ? (
            ...
            <>
              ...
              <p style={{ margin: '0 0 15px 0', fontSize: '0.9rem', color: 'var(--gris-texto)', fontWeight: 500 }}>Profesional de Enfermería</p>
            </>
          )}
        ...
        <div style={{ background: 'var(--blanco)', borderRadius: '20px', padding: '5px 15px', boxShadow: 'var(--shadow-soft)', marginBottom: '16px' }}>
          <div onClick={() => navigate('/profesional/datos-personales')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 5px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: '#f8fafc', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--verde-logo)', fontSize: '1.1rem' }}><i className="fa-regular fa-id-card"></i></div>
              <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--texto)' }}>Datos Personales</span>
            </div>
            <i className="fa-solid fa-chevron-right" style={{ color: '#cbd5e1' }}></i>
          </div>
          <div onClick={() => navigate('/profesional/servicios-ofrezco')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 5px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: '#f8fafc', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--verde-logo)', fontSize: '1.1rem' }}><i className="fa-solid fa-briefcase-medical"></i></div>
              <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--texto)' }}>Servicios que ofrezco</span>
            </div>
            <i className="fa-solid fa-chevron-right" style={{ color: '#cbd5e1' }}></i>
          </div>
          <div onClick={() => navigate('/profesional/resenas')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 5px', cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: '#f8fafc', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--verde-logo)', fontSize: '1.1rem' }}><i className="fa-solid fa-star" style={{ color: '#fbbf24' }}></i></div>
              <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--texto)' }}>Mis Reseñas y Puntuación</span>
            </div>
            <i className="fa-solid fa-chevron-right" style={{ color: '#cbd5e1' }}></i>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--blanco)', borderRadius: '20px', padding: '5px 15px', boxShadow: 'var(--shadow-soft)' }}>
          <div onClick={() => setModalVisible(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 5px', cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
